...
class Mapper:
    ...
    def __init__(self, data=[], epsilon=0.5, property_key="readingLevel", num_interval=3):
        self.data = data
        ...
        for i in range(n):
            ...
            for j in range(i+1, n):
                ...
                if not cluster.isdisjoint(nextCluster) and newNodes[names[i]] != newNodes[names[j]]:
                    edges.append((newNodes[names[i]], newNodes[names[j]], {
                        "id": id, "type": j_index}))
                    id += 1

        for node in newNodes.values():
            g.add_nodes_from([(node, node)])
    ...
    def clusterAverage(self, name, cluster, property_keys):
        ...
        numerical_variables = []
        clusterNode = Node(
            {'subreddit': cluster[0].subreddit_tag, 'name': name})
...
class EdgeMapper(Mapper):
    def __init__(self, edges, epsilon=0.5, property_key="readingLevel", num_interval=3):
...
class NodeMapper(Mapper):
    def __init__(self, edges, data, epsilon=0.5, property_key="readingLevel", num_interval=3):
        self.edges = edges
